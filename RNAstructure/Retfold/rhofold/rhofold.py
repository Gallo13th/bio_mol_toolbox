...
class RhoFold(nn.Module):
    ...
    def forward_one_cycle(self, tokens, rna_fm_tokens, recycling_inputs, seq, retention=False):
        '''
        Args:
        ...
        if retention:
            immediate_state = {'msa':{'kv':torch.tensor([0]).to(device),'scale':None},
                           'pair':{'kv':torch.tensor([0]).to(device),'scale':None}}
        else:
            immediate_state = {'msa':None,
                            'pair':None,}
    ...
    def forward(self,
        ...
        for _r in range(self.config.model.recycling_embedder.recycles):
            output, recycling_inputs = \
                self.forward_one_cycle(tokens, rna_fm_tokens, recycling_inputs, seq, retention)
