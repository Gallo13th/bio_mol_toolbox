...
def main(config):
    ...
    model = RhoFold(student_config)
    # model = RhoFold(rhofold_config)

    logger.info(f'    loading {config.ckpt}')
    # model.load_state_dict(torch.load(config.ckpt, map_location=torch.device('cpu'))['model'],strict=False)
    model.load_state_dict(torch.load('./pretrained/retnet_19.pt', map_location=torch.device('cpu')),strict=False)
    model.eval()
