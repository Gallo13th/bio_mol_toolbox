...
from rhofold.utils.lr_schedulers import AlphaFoldLRScheduler
from rhofold.utils.validation_metrics import drmsd
import copy
...
def hallucination_loss(pred,ref,site_h=None,pdb_ref=False):
    ...
    if not pdb_ref:
        ...
        ref_loss_fape = compute_fape(
                ...
                positions_mask=pt_mask.to(pred_dist_n.device),
                length_scale=1,
                l1_clamp_distance=100
            )
        ref_loss_fape = torch.sum(ref_loss_fape*dist_mask)/(torch.sum(dist_mask) + 1e-6)

        loss = ref_loss_fape # - 10 * torch.mean(plddt)
...
def main(config):
    ...
    # logger.info(f'    loading {config.main_ckpt}')
    # model.load_state_dict(torch.load(config.main_ckpt, map_location=torch.device('cpu'))['model'],strict=False)
    # model.eval()
    # # background_model
    # background = RhoFold(rhofold_config)
    # logger.info(f'    loading {config.background_ckpt}')
    # background.load_state_dict(torch.load(config.background_ckpt, map_location=torch.device('cpu')),strict=False)
    # background.eval()
    # ref_model
    reference = RhoFold(rhofold_config)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lowest_plddt",default=0.9)
    parser.add_argument("--de_novo",default=0)
